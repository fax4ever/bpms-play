...
public class SelectionProcessTest extends JbpmJUnitBaseTestCase {
	...
	public void before() {
		...
		createRuntimeManager("it/redhat/demo/v5/selection-process.bpmn2");
	...
	@Test
	public void test_promoted_promoted_rejected() {
		...
		// find task by pot owners
		hrTasks = taskService.getTasksAssignedAsPotentialOwner("trionfera", "en-UK");
		assertEquals(1, hrTasks.size());
		hrTask = hrTasks.get(0);
		assertEquals(Status.Ready, hrTask.getStatus());
		
		taskService.claim(hrTask.getId(), "trionfera");
		taskService.start(hrTask.getId(), "trionfera");
		
		HashMap<String,Object> taskResults = new HashMap<String, Object>();
		taskResults.put("feedback", "KO");
		
		// the HR trionfera reject the candidate
		taskService.complete(hrTask.getId(), "trionfera", taskResults);
		
		assertNodeTriggered(pi.getId(), "Human Resource Proposal", "Proposal not accepted");
		assertProcessInstanceCompleted(pi.getId());
		
	}
	
	@Test
	public void test_promoted_promoted_promoted() {
		...
		// find task by pot owners
		hrTasks = taskService.getTasksAssignedAsPotentialOwner("coppellardi", "en-UK");
		assertEquals(1, hrTasks.size());
		hrTask = hrTasks.get(0);
		assertEquals(Status.Ready, hrTask.getStatus());
		...
		// the HR trionfera reject the candidate
		taskService.complete(hrTask.getId(), "coppellardi", taskResults);
